...
def calculate_pol_E_T(MATERIAL, q_XYZ_list):
    ...
    photon_eigenvecs_list = physics.create_photon_eigenvec_list(
        q_XYZ_list, dielectric, born, V_PC, atom_masses)
    photon_eigenvals_list = physics.create_photon_eigenvals_list(
        q_XYZ_list, dielectric, born, V_PC, atom_masses)
    ...
    physics.check_photon_eigenvectors(
        q_XYZ_list, dielectric, born, V_PC, atom_masses)
    ...
    o_xi_vecs_list = physics.create_xi_vecs(
        born, ph_eigenvectors_o, atom_masses)
    ...
    polariton_T_mat_list = np.zeros(
        (n_q, 2*(num_modes - 3 + 2), 2*(num_modes - 3 + 2)), dtype=complex)
    ...
    for q in range(n_q):
        ...
        # KP
        # [E_mat, T_mat] = uv.UVMatrix(h_mat).get_T_matrix()
        ...
        T_invgT_inv_dag_mat = np.matmul(
            T_mat_inv, np.matmul(g_mat, T_mat_inv_dag))
        ...
        if not np.allclose(T_invgT_inv_dag_mat, g_mat, atol=atol_param):
            ...
            print(np.array_str(T_invgT_inv_dag_mat,
