...
def create_xi_vecs(born, eigenvectors, atom_masses, no_born=False):
    """
            ...
            for alpha in range(3):
                for j in range(num_atoms):
                    if no_born == False:
                        ...
                        xi_vecs[k][nu][alpha] += (atom_masses[j])**(-0.5)\
                            * np.matmul(np.eye(3), eigenvectors[k][nu][j])[alpha]
...
def create_photon_eigensys(q, dielectric, born, V_PC, atom_masses):
    ...
    orthog_basis = create_perp_orthonormal_basis(q_dielectric)

    # compute e_nu^* . dielectric . e_nup
    projected_dielectric = np.zeros((2, 2), dtype=np.complex128)
    ...
    for lam in range(2):
        for i in range(3):
            for lamp in range(2):
                ...
                basis[lam][i] += eigen_vecs_proj_dielectric[lam][lamp] * \
                    orthog_basis[lamp][i]
...
def check_photon_eigenvectors(q_XYZ_list, dielectric, born, V_PC, atom_masses):
    ...
    for q in range(len(q_XYZ_list)):
        ...
        eigen_sys_q = create_photon_eigensys(
            q_XYZ_list[q], dielectric, born, V_PC, atom_masses)
        eigen_sys_minus_q = create_photon_eigensys(
            -1*q_XYZ_list[q], dielectric, born, V_PC, atom_masses)
        ...
        if (
            ...
            np.abs(np.dot(q_dir, np.matmul(
                dielectric, basis_q[1]))) > 10**(-8)
        ):
...
def create_photon_eigenvec_list(q_list, dielectric, born, V_PC, atom_masses):
    ...
    for q in range(len(q_list)):
        ...
        out.append(create_photon_eigensys(
            q_list[q], dielectric, born, V_PC, atom_masses)[0])
...
def create_photon_eigenvals_list(q_list, dielectric, born, V_PC, atom_masses):
    ...
    for q in range(len(q_list)):
        ...
        eigen_sys = create_photon_eigensys(
            q_list[q], dielectric, born, V_PC, atom_masses)
        out.append([
            eigen_sys[1],
            eigen_sys[2]
        ])
...
def rot_matrix(direction, theta):
	...
	rot = [
            [ct + u1*u1*(1 - ct), u1*u2*(1 - ct) -
             u3*st, u1*u3*(1-ct) + u2*st],
