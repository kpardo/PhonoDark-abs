...
def run_phonopy(phonon_file, k_mesh):
    ...
    eigenvectors = np.zeros(
        (n_k, num_modes, num_atoms, 3), dtype=np.complex128)
...
def get_phonon_file_data(phonon_file, material):
    ...
    recip_lat_a = 2*PI*(np.cross(b_vec, c_vec)) / \
        (np.matmul(a_vec, np.cross(b_vec, c_vec)))
    recip_lat_b = 2*PI*(np.cross(c_vec, a_vec)) / \
        (np.matmul(b_vec, np.cross(c_vec, a_vec)))
    recip_lat_c = 2*PI*(np.cross(a_vec, b_vec)) / \
